...
from app.schemas.agent import AgentCreditLogListOut, AgentOverviewOut, AgentRedeemKeyListOut
...
@router.get("/agents/{user_id}/overview", response_model=AgentOverviewOut)
def admin_agent_overview(
    user_id: str,
    _user: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    return get_agent_redeem_overview(db, agent=get_agent_user(db, user_id))


@router.get("/agents/{user_id}/redeem-keys", response_model=AgentRedeemKeyListOut)
def admin_agent_redeem_keys(
    user_id: str,
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=200),
    batch_no: Optional[str] = Query(None),
    ...
    return list_agent_redeem_keys(
        db,
        agent=get_agent_user(db, user_id),
        page=page,
        page_size=page_size,
        batch_no=batch_no,
        redeem_key=redeem_key,
...
@router.get("/agents/{user_id}/credit-logs", response_model=AgentCreditLogListOut)
def admin_agent_credit_logs(
    user_id: str,
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
